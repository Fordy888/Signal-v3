...
## Repair deployment — 4 September 2026

Render build `bld-dad61vlg1s2s73f6rudg` checked out commit `088dfeef9438b78bccd2a7cf9635222f4d7c024a`, ran the source-controlled build command and passed **121/121 tests**. Render reported `Build succeeded | Latest`. The service's actual deployed SHA and saved command still require a fresh settings/overview read before a canary can be authorised; build success alone is not CANARY VERIFIED.

A fresh Render settings read confirmed `088dfeef9438b78bccd2a7cf9635222f4d7c024a` as the last successfully deployed commit. The saved command is bound to that exact SHA, release ID `ai-adoption-v1-proof-0047`, approved checksum `c43ec4b92fa8bc815ff09538b38e5ee5e32a3882586f90195d6166247c408a06`, the versioned manifest, date-resolved image record and `--locked-edition 47`. It retains `--proof --release-canary --enhanced --alive-moment`, contains no `--send`, and targets only the configured proof recipient. A fresh run has not yet occurred, so CANARY VERIFIED remains false.

The fresh locked Edition 0047 canary was manually triggered at approximately `2026-09-04T06:17:25Z` from that verified proof-only command. The Render run identifier, terminal status, release receipt and Gmail delivery were not yet available at this checkpoint; no canary state is inferred from the trigger alone.

The active run list records the new manual run beginning `2026-09-04T06:17:25Z`. Its initial logs confirm proof mode, recipient `paul.ford@gmail.com` only, recipient integrity `1 unique valid emails`, next edition `0047`, deployed code `088dfeef9438b78bccd2a7cf9635222f4d7c024a`, and source fetching in progress. No delivery, release-identity match or terminal status was yet established.

At 4 minutes 37 seconds the run remained active. The available log window showed source collection continuing and no delivery attempt, release receipt or terminal status. CANARY VERIFIED remained false at this checkpoint.

At 10 minutes 31 seconds the fresh run was still active. Render had not displayed a terminal status and no delivery or receipt evidence had yet been captured, so CANARY VERIFIED remained false.

The run finished successfully after 10 minutes 51 seconds. Render logs show 436 items scored, 197 above threshold, checksum-locked Edition 0047 loaded, 20,107 HTML characters produced, and HTML SHA-256 `c43ec4b92fa8bc815ff09538b38e5ee5e32a3882586f90195d6166247c408a06`. The pre-send gate passed Edition 0047/date/content/source/recipient/reply-to checks. Release identity reported `TARGET MATCH` for release `ai-adoption-v1-proof-0047`, renderer `enhanced-v4-focus-numbers`, branch `master`, commit `088dfeef9438`, service `crn-d8ouk0bsq97s73fgc36g`, and the approved proof checksum.

The run delivered 1/1 to `paul.ford@gmail.com` through Resend ID `b14c6716-0a39-4401-8455-0e869d9ddb1d`, reported one DTL PL delivery mapping for Edition 0047, saved a successful canary receipt and sent that receipt to Paul. Render exited successfully with `Sent: 1/1, Failed: 0`. This establishes successful production canary execution and target compliance. **CANARY VERIFIED remains pending** until the Resend record and actual Gmail copy are independently inspected.

An independent Resend read returned message ID `<010601a06b1a6e2c-29ff8e11-409d-48ca-881a-d13243cee011-000000@ap-northeast-1.amazonses.com>`, sender `DTL Signal <signal@signal.dtlc.ai>`, recipient and reply-to `paul.ford@gmail.com`, subject `[PROOF] DTL Signal | Final Founder-Led Format | Edition 0047`, and provider status `delivered`. The stored body begins with the approved Edition 0047 Founder’s Note and all-AI Newsroom/Focus copy. Provider delivery is verified; actual Gmail rendering remains outstanding.

Gmail independently shows the Signal message `1a06b1a7101d011a` and canary receipt `1a06b1a7c29ef9e8`, both delivered at 06:28 UTC. The full Gmail body contains the approved Founder’s Note, five Newsroom stories, five Focus figures, interpretation/actions/counter/watch sequence, `REMEMBER THE WORLD`, the full Norderney image, Dietmar Rabich/Wikimedia/CC BY-SA 4.0 attribution, and the Dad Joke after the photograph. Reader-visible Gmail content is verified.

...
class ReleaseIdentityTests(unittest.TestCase):
    ...
    def test_render_blueprint_uses_v4_daily_command_and_build_gate(self):
        ...
        self.assertEqual(
            ...
            # Subscriber delivery is OFF by standing instruction: the production cron
            # runs the full pipeline and stops short of sending.
        ...
        self.assertEqual(env["SIGNAL_ALIVE_MOMENT_PATH"], "data/alive_moment.json")
        proof_service = next(
            item for item in blueprint["services"] if item["name"] == "dtl-signal-proof"
        )
        self.assertEqual(
            proof_service["startCommand"],
            # A contemporary proof, not a replay of the frozen 0043 canary.
            "python -m src.main --proof --enhanced --alive-moment",
        )

...
# ─── Content-mix shortfalls (formerly hard-fail quota gates) ───
#
# These used to `raise JudgementPlanError` and fail the whole edition whenever a
# content-mix quota was unmet (e.g. "requires exactly 3 AI_BUSINESS and 2
# MAJOR_BUSINESS per section"). That made an otherwise-qualified edition fail on
# a supply problem in the source crawl.
#
# They are now recorded, not raised. The allocator returns whatever it could
# verify, and `section_policy` decides what that means: an OPTIONAL section that
# cannot meet its evidence bar is omitted and the reason logged; MANDATORY
# sections are still enforced, and still halt. The bar is never lowered — the
# section simply does not appear.
_SHORTFALLS: list[str] = []


def _note_shortfall(message: str) -> None:
    """Record a content-mix shortfall without failing the edition."""
    log.warning("Content-mix shortfall (section may be omitted): %s", message)
    _SHORTFALLS.append(message)


def drain_shortfalls() -> list[str]:
    """Return and clear shortfalls recorded during this planning run."""
    recorded = list(_SHORTFALLS)
    _SHORTFALLS.clear()
    return recorded

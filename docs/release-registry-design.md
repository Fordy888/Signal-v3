...
The dedicated service's first Edition 0049 proof preflight held before rendering and registry insertion because only two Focus sources survived as verified numeric AI adoption against the minimum of three. A bounded adoption-feed audit identified deterministic evidence attrition rather than a policy shortage: an Inside Retail source stated that Australian retailers already use AI in everyday operations and supplied 87 `per cent` privacy concern and 5 `per cent` trust, but an unrelated sentence containing `might` vetoed the whole record and the numeric extractor did not recognise `per cent`. The correction recognises source-owned `per cent` and scopes hypothetical rejection to the affected sentence; it does not lower the 3-of-5 Focus adoption minimum. Non-reader logs now expose the numeric, adoption, numeric-adoption and industry-impact pool counts.

The same preflight exposed a separate durability gap: the dedicated service started without local URL, joke or image history because Render's filesystem is ephemeral. Registry preparation now fails closed unless it can merge a source-controlled cutover seed with delivered Postgres metadata. The seed contains the exact ten Edition 0047 delivered source URLs and the approved Edition 0047/0048 governed image records; no joke identifier is invented for legacy jokes absent from the governed catalogue. Recent delivered production source URLs feed cross-day deduplication, while delivered proof and production joke/image identities feed non-repeat validation. Every newly locked release persists its source URLs, joke ID and full governed image record, so preflight memory survives deploys, restarts and both services.
...
The current candidate readiness gate comprises **186 tests across 17 independently bounded modules** in both the integration checkout and a fresh detached worktree, with the five production-only database tests skipped in each general run and then executed successfully against checksum-migrated isolated PostgreSQL databases in both checkouts. The real Postgres gate proves recent production source URLs survive restart, proof-only sources cannot poison production dedup, and delivered proof/production joke and image identities remain available for non-repeat checks. Edition 0049 has a date-resolved, public-domain NASA Great Barrier Reef record with hosted bytes locked to SHA-256 `1b49747f4a2c72a4673c158bef5710c116928dee3ed7e9ba72f552ffc5d41727`. Registry preflight requires the hosted HTTPS asset, image content type and exact byte checksum before freezing; delivery uses the immutable record and performs no image fetch. These are tested candidate facts until the durable-history change set is committed and deployed; scheduled-time subscriber evidence still requires a new future edition and a separate activation decision.

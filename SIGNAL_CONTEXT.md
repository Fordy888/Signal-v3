...
If an independently verified `AI_BUSINESS` Focus item fails to make its AI subject and business consequence explicit in reader-visible copy, the third attempt may replace only its `meaning` with a clean sentence copied from that same source’s evidence. The source, defining figure and classification remain fixed. Cross-source wording, invented copy, internal source IDs and any change to `MAJOR_BUSINESS` items remain prohibited and hold the edition.

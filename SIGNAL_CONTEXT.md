...
Source allocation is deterministic before planning: the model may write only from the five source IDs assigned to each section. Adoption must dominate the ten items. The current conservative implementation safeguard requires at least eight `AI_ADOPTION` items and permits no more than two `AI_INDUSTRY_IMPACT` items, with at least four adoption items in each section. This 8/2 threshold is an implementation interpretation of the approved adoption-first direction and remains reviewable through the new proof; it must not be represented as a separately approved editorial quota.
